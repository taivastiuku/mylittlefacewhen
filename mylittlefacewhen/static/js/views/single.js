...
window.SingleView = Backbone.View.extend({
  ...
  initialize: function(options) {
    ...
    $(window).scrollTop(0);
    KeyboardJS.on('r', function(event) {
      return _this.random(event);
    });
    KeyboardJS.on('left', function(event) {
      return _this.previous(event);
    });
    KeyboardJS.on('right', function(event) {
      return _this.next(event);
    });
    KeyboardJS.on('h', function(event) {
      return _this.previous(event);
    });
    KeyboardJS.on('l', function(event) {
      return _this.next(event);
    });
    KeyboardJS.on('esc', function(event) {
